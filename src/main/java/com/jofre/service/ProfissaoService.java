package com.jofre.service;

import java.util.List;
import java.util.Map;
import java.util.Set;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import com.jofre.datatables.Datatables;
import com.jofre.datatables.DatatablesColunas;
import com.jofre.domain.Profissao;
import com.jofre.repository.ProfissaoRepository;

import jakarta.servlet.http.HttpServletRequest;

@Service
public class ProfissaoService {

	@Autowired
	private ProfissaoRepository repository;
	
	@Autowired
	private Datatables datatables;

	@Transactional(readOnly = false)
	public void salvar(Profissao profissao) {
		repository.save(profissao);		
	}
	
	@Transactional(readOnly = true)
	public Map<String, Object> buscarProfissoes(HttpServletRequest request) {
		datatables.setRequest(request);
		datatables.setColunas(DatatablesColunas.PROFISSOES);
		Page<?> page = datatables.getSearch().isEmpty()
				? repository.findAll(datatables.getPageable())
				: repository.findAllByNome(datatables.getSearch(), datatables.getPageable());
		return datatables.getResponse(page);
	}
	
	@Transactional(readOnly = true)
	public Profissao buscarPorId(Long id) {
		return repository.findById(id).get();
	}

	@Transactional(readOnly = false)
	public void remover(Long id) {
		repository.deleteById(id);
	}
	
	@Transactional(readOnly = true)
	public List<String> buscarProfissaoByTermo(String termo) {
		return repository.findProfissoesByTermo(termo);
	}
	
	@Transactional(readOnly = true)
	public Set<Profissao> buscarPorTitulos(String[] nomes) {
		return repository.findByNomes(nomes);
	}
	
	@Transactional(readOnly = true)
	public List<Profissao> buscarTodos() {
		return repository.findAll();
	} 
	
}
